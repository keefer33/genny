...
- Generated by `vite-plugin-pwa` from `vite.config.ts` manifest config
...
3. **Theme Colors**: Matches your app's theme
4. **Standalone Mode**: Runs like a native app when installed
...
Edit `vite.config.ts` → `VitePWA` → `manifest` to customize:
...
- Verify manifest content in `vite.config.ts` is valid

### Icons Not Displaying

- Ensure icons exist in `/public/icons/`
- Check icon paths in `vite.config.ts` manifest config
