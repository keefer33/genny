...
const useTagStoreBase = create<TagState>((set, get) => ({
  ...
  loadTags: async (userId) => {
    ...
    try {
      const json = await authFetchJson<{ tags: UserTag[] }>(`${endpoint}/user/tags`, undefined, {
  ...
  createTag: async (userId, tagName) => {
    ...
    try {
      const json = await authFetchJson<{ tag: UserTag }>(
        `${endpoint}/user/tags`,
        {
          method: "POST",
  ...
  getFileTags: async (fileId) => {
    ...
    try {
      const json = await authFetchJson<{ tags: UserFileTag[] }>(
        `${endpoint}/user/tags/files/${encodeURIComponent(fileId)}`,
        undefined,
